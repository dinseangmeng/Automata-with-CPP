...
void DisplayDataInSql(MYSQL *connector){
    ...
    if(connector){
        ...
        if(!state){
            ...
            SubHeader("ALL DATA IN DATABASE",5,50,7);
...
void insertToDataBase(struct FA FA1,MYSQL* connecter,string label){
    ...
    if(!State){
        SubHeader("SAVE SUCCESSFUL",5,50,7,'g');
    }else{
        SubHeader("FAILED TO SAVE",5,50,7,'r');
    }
}
void deleteFA(MYSQL *connector,string id){
    ...
    if(!State){
        SubHeader("DELETE SUCCESSFULL",5,50,7,'g');
        text="ALTER TABLE FA AUTO_INCREMENT=1;";
        mysql_query(connector,text.c_str());
    }else{
        SubHeader("FAILED TO DELETE",5,50,7,'r');
    }
}
